...
pub struct TerminalLineEditor {
    editor: Editor<FallbackRustylineHelper, DefaultHistory>,
    history: Vec<String>,
}
...
impl TerminalLineEditor {
    pub fn new() -> HarnessResult<Self> {
        let mut editor =
            Editor::<FallbackRustylineHelper, DefaultHistory>::new().map_err(|err| {
                HarnessError::External(format!("failed to initialize terminal line editor: {err}"))
            })?;
        editor.set_helper(Some(FallbackRustylineHelper::new()));
...
pub struct FallbackRustylineHelper {
    engine: CompletionEngine,
    catalog: CommandCatalog,
    state: Box<dyn CompletionStateView>,
    repo: Option<PathBuf>,
    last_replacement_end: Cell<usize>,
}

impl FallbackRustylineHelper {
    pub fn new() -> Self {
        Self::with_state(Box::new(EmptyCompletionStateView))
    }

    pub fn with_state(state: Box<dyn CompletionStateView>) -> Self {
        Self {
            engine: CompletionEngine::new(),
            catalog: build_cli(),
            state,
            repo: None,
            last_replacement_end: Cell::new(0),
        }
    }
}

impl Default for FallbackRustylineHelper {
    fn default() -> Self {
        Self::new()
    }
}

impl Completer for FallbackRustylineHelper {
    type Candidate = Pair;
    ...
    fn complete(
        ...
        let completions = self
            .engine
            .complete(line, pos, &context)
            .map_err(|err| ReadlineError::Io(std::io::Error::other(err.to_string())))?;
        self.last_replacement_end.set(completions.replacement_end);
        let candidates = completions
            ...
            .collect();
        Ok((completions.replacement_start, candidates))
    }

    fn update(&self, line: &mut LineBuffer, start: usize, elected: &str, cl: &mut Changeset) {
        let end = self.last_replacement_end.get().max(line.pos());
        line.replace(start..end, elected, cl);
    }
}

impl Hinter for FallbackRustylineHelper {
    type Hint = String;
}

impl Highlighter for FallbackRustylineHelper {}

impl Validator for FallbackRustylineHelper {}

impl Helper for FallbackRustylineHelper {}

struct EmptyCompletionStateView;

impl CompletionStateView for EmptyCompletionStateView {
...
mod tests {
    use super::*;
    use crate::completion::CompletionStateView;
    use crate::domain::{RunId, Task, TaskId, TaskStatus, Ticket, TicketId, TicketStatus};
    ...
    #[test]
    fn run_with_input_keeps_piped_bufread_behavior() {
        let service = StubService;
        let runtime = CommandRuntime::new(&service);
        ...
        let exit = run_with_input(&mut stdin, &mut stdout, &mut stderr, &runtime);

        assert_eq!(exit.code(), 0);
        let stdout = String::from_utf8(stdout).unwrap();
        assert!(stdout.contains("harness interactive mode"));
    ...
    #[test]
    fn fallback_completer_delegates_to_completion_engine() {
        let helper = FallbackRustylineHelper::with_state(Box::new(RecordingStateView::default()));
        let history = DefaultHistory::new();
        let context = Context::new(&history);

        let (start, candidates) = helper.complete("t", 1, &context).unwrap();
        ...
        assert!(
            candidates
                .iter()
                .any(|candidate| candidate.replacement == "ticket")
        );
    }

    #[test]
    fn fallback_completer_uses_completion_state_view_for_ids() {
        let state = RecordingStateView::default();
        let task_queries = state.task_queries.clone();
        let helper = FallbackRustylineHelper::with_state(Box::new(state));
        let history = DefaultHistory::new();
        let context = Context::new(&history);

        let (_start, candidates) = helper
            .complete("task get task_", "task get task_".len(), &context)
            .unwrap();

        assert_eq!(task_queries.get(), 1);
        assert_eq!(candidates[0].replacement, "task_01ARZ3NDEKTSV4RRFFQ69G5FAV");
    }

    #[test]
    fn fallback_does_not_reference_persistent_history_apis_or_files() {
        let source = include_str!("fallback.rs");
        assert!(!source.contains(concat!("load", "_history")));
        assert!(!source.contains(concat!("save", "_history")));
        assert!(!source.contains(concat!("append", "_history")));
        assert!(!source.contains(concat!("history", "_file")));
    }
    ...
    struct RecordingStateView {
        task_queries: std::rc::Rc<Cell<usize>>,
    }

    impl CompletionStateView for RecordingStateView {
        fn tasks_for_completion(
            &self,
            _scope: TaskCompletionScope,
        ) -> HarnessResult<Vec<TaskCompletionItem>> {
            self.task_queries.set(self.task_queries.get() + 1);
            Ok(vec![TaskCompletionItem {
        ...
        fn tickets_for_completion(
            ...
            Ok(vec![TicketCompletionItem {
                id: TicketId::parse("ticket_01CRZ3NDEKTSV4RRFFQ69G5FAV").unwrap(),
                task_id: TaskId::parse("task_01ARZ3NDEKTSV4RRFFQ69G5FAV").unwrap(),
                run_id: RunId::parse("run_01ERZ3NDEKTSV4RRFFQ69G5FAV").unwrap(),
